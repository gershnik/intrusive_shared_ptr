...
`ref_counted` is meant to be used as a base class for any class you want to make reference-counted.
It is supposed to be used in conjunction with the [`refcnt_ptr`](refcnt_ptr.md) specialization of `intrusive_shared_ptr`, but
of course, it can also be used via manual reference counting calls or other smart pointers if desired.
...
* Many methods of `ref_counted` are accessed via CRTP calls to `Derived`. This allows "overriding" them in your derived class to modify or augment their functionality.
  * In particular, the `destroy()` member function is called to actually destroy the instance when the reference count drops to 0. The base `ref_counted` implementation invokes `delete` on the `Derived` pointer. Overriding this function in a derived class allows you to handle objects allocated in a different way.
...
- `template<class Derived> weak_ref_counted` is a `ref_counted` with the `ref_counted_flags::provide_weak_references` flag.
- `template<class Derived, class CountType=...> ref_counted_st` is a `ref_counted` with the `ref_counted_flags::single_threaded` flag.
- `template<class Derived> weak_ref_counted_st` is a `ref_counted` with both `provide_weak_references` and `single_threaded` 
flags set.
...
You probably want to make your destructor private or protected (if your class is itself a base). Reference-counted objects should only be destroyed via reference counting, not via direct `delete` calls.
...
- **Never increment the reference count (by creating `refcnt_ptr` from `this` for example or any other means) in the destructor.** Doing so will trigger an assert in debug builds and cause undefined behavior in release ones.
  - It is generally OK to create weak references to `this` in the destructor.
- Never give out `this` (by creating `refcnt_ptr` from `this` for example or any other means) or a weak reference to `this` in a constructor **if any subsequent code in the constructor can exit via an exception**. If such an exception is thrown, your object will fail to be created, leaving the outside code holding an invalid pointer. This is a general C++ rule. Usage of reference counting does not negate it.
  - It is OK and supported to do reference counting in a constructor **if any subsequent code in the constructor does not exit via exception**.
- Your destructor must not throw exceptions. Another general C++ rule made even more relevant here, since the reference counting machinery in `ref_counted` does not expect an exception to be thrown and will break in this case.
- If your class *is* a base, then the destructor needs to be virtual - `ref_counted` will invoke the destructor of the `Derived` template argument. (If you propagate the most derived class as the `Derived` argument then this obviously does not apply).


### Limitations

When not using weak references, the implementation of `ref_counted` is standard-compliant and portable (modulo bugs and compiler deficiencies, of course). The only practical limitation is that the total number of outstanding references to an object must be between 0 and `std::numeric_limits<CountType>::max()`.

When using weak references, `ref_counted` relies on multiplexing pointers and the reference count in the same memory location. This mechanism should work provided:
* The platform uses 2's complement arithmetic 
* `alignof(intptr_t) > 1`
* When cast to `uintptr_t` the value of a pointer to an object with alignment bigger than 1 has the lowest bit 0 (e.g., is even).
...
* `ref_counted_base` - a synonym for itself. This allows one to refer to the type from its derived types without caring about specific template arguments, like `derived::ref_counted_base`.
...
* Protected default constructor. The reference count is set to 1 in it.
* Protected destructor. 
* Protected `void destroy() const noexcept`. Called when the reference count drops to 0 to destroy the object. The default implementation calls `delete` on the derived class pointer. Can be overridden in a derived class.
* Public `void add_ref() const noexcept`. Increments the reference count. Can be overridden in a derived class.
* Public `void sub_ref() const noexcept`. Decrements the reference count and destroys the object if it reaches 0. Can be overridden in a derived class.
* If the class supports weak references, then two additional public methods are available <br/>
  ...
  These methods are **not** `noexcept`. The weak reference "control block" is created lazily when a weak reference is requested for the first time. If memory allocation or the customized weak reference class constructor throws, these methods will throw. Subsequent calls will be `noexcept`. 
* Protected `const weak_value_type * get_weak_value() const`. Only meaningful if the class supports weak references. This is the actual method that retrieves a raw pointer to the weak reference used to implement `get_weak_ptr`. Not `noexcept`. Can be overridden in a derived class.
* Protected `weak_value_type * make_weak_reference(intptr_t count) const`. Only meaningful if the class supports weak references. This method is called to create a weak reference (control block) when one is needed for the first time. The returned pointer has its own reference count already incremented (as appropriate for a method returning a raw pointer). Can be overridden in a derived class.


### Customizing weak reference type
<a id="markdown-customizing-weak-reference-type" name="customizing-weak-reference-type"></a>

It is possible to customize the weak reference type used by your class by "overriding" `make_weak_reference`.
...
If such a function exists, it will be called instead of the one provided by `ref_counted` itself, and the type it returns will be the weak reference type.
...
Internally, `weak_reference` is a "control block" for a `ref_counted`. It is reference counted itself and also manages the count for the referenced object. `weak_reference` objects are created on-demand when the first weak reference is requested from `ref_counted`.
...
In particular, the `destroy()` member function is called to actually destroy the instance when the reference count drops to 0. The base `weak_reference` implementation invokes `delete` on your class pointer. Overriding this function in a derived class allows you to handle objects allocated in a different way.
...
Note that `const`-ness propagates between strong and weak pointers. A strong pointer to const yields a weak pointer to const and vice versa. 
...
* Protected destructor. 
* Protected `void destroy() const noexcept`. Called when the object's own reference count drops to 0 to destroy the object. The default implementation calls `delete` on the derived class pointer. Can be overridden in a derived class.
* Public `void add_ref() const noexcept`. Increments the object's own reference count. Can be overridden in a derived class.
* Public `void sub_ref() const noexcept`. Decrements the object's own reference count and destroys the object if it reaches 0. Note that the `Owner` always holds a reference to its `weak_reference` (control block), so it will only be destroyed after the `Owner` object. Can be overridden in a derived class.
* Public <br/>
...
This class publicly derives from a non-reference counted class `T` and a `ref_counted`. The remaining template parameters are forwarded to `ref_counted`.
...
This class stores (wraps) a member of class `T` and derives from `ref_counted`. The remaining template parameters are forwarded to `ref_counted`.
